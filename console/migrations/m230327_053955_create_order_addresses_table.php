 ...
 */
class m230327_053955_create_order_addresses_table extends Migration
{
    /**
     * {@inheritdoc}
     */
    public function safeUp()
    {
        ...
        //adds foreign key for table {{orders}}
        $this->addForeignKey(
    ...
    {
        // drops the foreign key for table `{{%orders}}`
        $this->dropForeignKey(
            '{{%fk-order_addresses-order_id}}',
            '{{%order_addresses}}'
        );

        // drops the index for column order_id
        $this->dropIndex(
            '{{%idx-order_addresses-order_id}}',
            '{{%order_addresses}}'
        );
